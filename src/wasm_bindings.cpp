...
int detectLandmarksAsync(uint8_t* imageData, int width, int height) {
    ...
    std::thread([promiseId, imageCopy, width, height]() {
        ...
        EM_ASM({
            _landmarkDetectionComplete($0, $1, $2);
...
EMSCRIPTEN_BINDINGS(face_landmark_module) {
    ...
    emscripten::function("isThreadingSupported", &isThreadingSupported);
    emscripten::function("getNumHardwareThreads", &getNumHardwareThreads);
}
